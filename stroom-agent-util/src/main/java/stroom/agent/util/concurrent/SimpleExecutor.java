package stroom.agent.util.concurrent;
...
public class SimpleExecutor {
			...
			executorService.execute(new Runnable() {
				...
				public void run() {
					try {
						...
						LOGGER
								.error(
										"run() - Uncaught exception from execution",
										th);
					} finally {
	...
	public void waitForComplete() {
		...
		while (!executorService.isTerminated()
				&& executorCompleteCount.get() < executorSubmitCount.get()) {
			defaultShortSleep();
	...
	public String toString() {
		return "SimpleExecutor(" + threadCount + ") progress="
				+ executorCompleteCount + "/" + executorSubmitCount;
	}
