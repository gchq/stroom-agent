...
import stroom.agent.util.io.FilterOutputStreamProgressMonitor;
import stroom.agent.util.io.StreamProgressMonitor;
import stroom.agent.util.io.StreamUtil;
import stroom.agent.util.io.WrappedOutputStream;
...
public class StroomZipOutputStream implements Closeable {
	...
	public StroomZipOutputStream(final File file, final Monitor monitor, final boolean monitorEntries) throws IOException {
		...
		zipOutputStream = new ZipOutputStream(new FilterOutputStreamProgressMonitor(new FileOutputStream(lockFile),
	...
	public StroomZipOutputStream(final OutputStream outputStream, final Monitor monitor) throws IOException {
		...
		lockFile = null;
		streamProgressMonitor = new StreamProgressMonitor(monitor, "Write");
		zipOutputStream = new ZipOutputStream(new FilterOutputStreamProgressMonitor(new BufferedOutputStream(
				outputStream), streamProgressMonitor));
		stroomZipNameSet = new StroomZipNameSet(false);
	}
