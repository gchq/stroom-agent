...
public class StroomStreamException extends RuntimeException {
	...
	private static StroomLogger LOGGER = StroomLogger
			.getLogger(StroomStreamException.class);
	...
	private static String buildMessage(final StroomStatusCode stroomStatusCode,
			final Object[] args) {
	...
	private static RuntimeException unwrap(final Throwable ex) {
		if (ex instanceof ZipException) {
			throw new StroomStreamException(
					StroomStatusCode.COMPRESSED_STREAM_INVALID, ex.getMessage());
		}
	...
	public static int checkConnectionResponse(final HttpURLConnection connection) {
		...
		try {
			...
			stroomStatus = connection.getHeaderFieldInt(
					StroomHeaderArguments.STROOM_STATUS, -1);
	...
	public static int sendErrorResponse(
			final HttpServletResponse httpServletResponse, Exception exception)
			throws IOException {
		...
		LOGGER.error("sendErrorResponse() - " + stroomStatusCode.getHttpCode()
				+ " " + message);

		httpServletResponse.setHeader(StroomHeaderArguments.STROOM_STATUS,
	...
	protected static void unwrapMessage(final StringBuilder stringBuilder,
			final Throwable th, final int depth) {
		if (depth == 0 || th == null) {
			return;
		}
