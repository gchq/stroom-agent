...
import stroom.agent.util.io.InitialByteArrayOutputStream;
import stroom.agent.util.io.InitialByteArrayOutputStream.BufferPos;
...
import stroom.agent.util.task.TaskScopeContextHolder;
...
public abstract class StroomZipRepositoryProcessor {
	...
	/**
	 * Process an STROOM zip repository,
	 *
	...
	private void fileScan(final StroomZipRepository stroomZipRepository, final File file) {
		...
		try {
			...
			// Small bit if blocking code to add the idea to the map of lists
