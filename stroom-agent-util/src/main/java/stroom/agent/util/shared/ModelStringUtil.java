...
public final class ModelStringUtil {
	...
	private static Divider SIZE_DIVIDER = new Divider(1, "", new Divider(1000,
			"K", new Divider(1000, "M", new Divider(1000, "G", new Divider(
					1000, "T", null)))));

	private static Divider BYTE_SIZE_DIVIDER = new Divider(1, "", new Divider(
			1, "b", new Divider(1, SUFFIX_BYTES, new Divider(BYTE_DIV,
					SUFFIX_KB, new Divider(BYTE_DIV, SUFFIX_MB, new Divider(
							BYTE_DIV, SUFFIX_GB, null))))));
	...
	private static Divider TIME_SIZE_DIVIDER_FORMAT = new Divider(1, " ms",
			new Divider(1000, " s", new Divider(60, " m", new Divider(60, " h",
					new Divider(24, " d", null)))));

	private static Divider TIME_SIZE_DIVIDER_PARSE = new Divider(1, "", new Divider(1, " ms",
			new Divider(1000, " s", new Divider(60, " m", new Divider(60, " h",
					new Divider(24, " d", null))))));

	public final static String formatNumberString(final double number,
			final Divider divider) {
		final double nextNumber = number / divider.div;
		if ((divider.next == null)
				|| (nextNumber < divider.next.div)) {
	...
	public final static Long parseNumberString(final String str)
			throws NumberFormatException {
		return parseNumberString(str, SIZE_DIVIDER);
	}

	public final static Long parseByteSizeString(final String str)
			throws NumberFormatException {
		return parseNumberString(str, BYTE_SIZE_DIVIDER);
	}

	public final static Long parseDurationString(final String str)
			throws NumberFormatException {
		return parseNumberString(str, TIME_SIZE_DIVIDER_PARSE);
	}

	public final static Integer parseNumberStringAsInt(final String str)
			throws NumberFormatException {
		...
		if (num.longValue() > Integer.MAX_VALUE) {
			throw new NumberFormatException(str
					+ " is too big for an int.  (Max value "
					+ formatCsv(Integer.MAX_VALUE) + " and you number was "
					+ formatCsv(num) + ")");
		}
		if (num.longValue() < Integer.MIN_VALUE) {
			throw new NumberFormatException(str
					+ " is too small for an int.  (Min value "
					+ formatCsv(Integer.MIN_VALUE) + " and you number was "
					+ formatCsv(num) + ")");
		}
		return num.intValue();
	}
	 ...
	 * @throws NumberFormatException
	 */
	public final static Long parseNumberString(String str, Divider divider)
			throws NumberFormatException {
		if (str == null) {
			return null;
		}
	...
	public static Comparator<String> pathComparator() {
		return new Comparator<String>() {
			...
			public int compare(final String o1, final String o2) {
				...
				for (int i = 0; i < min; i++) {
					final int r = ((Character) o1.charAt(i)).compareTo(o2
							.charAt(i));
					if (r != 0) {
						return r;
					}
