 ...
 * <p>
 * <strong>Text</strong>: If the number of pattern letters is 4 or more, the
 ...
 * <p>
 * <strong>Year</strong>: Numeric presentation for year and weekyear fields are
 * handled specially. For example, if the count of 'y' is 2, the year will be
 ...
 * <p>
 * <strong>Zone</strong>: 'Z' outputs offset without a colon, 'ZZ' outputs the
...
public class DateTimeFormat {
	...
	private static final Map<String, DateTimeFormatter> cPatternedCache = new ConcurrentHashMap<String, DateTimeFormatter>(
			7);
	/** Maps patterns to formatters, patterns don't vary by locale. */
	...
	// -----------------------------------------------------------------------
	/**
	 * Factory to create a formatter from a pattern string. The pattern string
	 * is described above in the class level javadoc. It is very similar to
	 * SimpleDateFormat patterns.
	 * <p>
	 * The format may contain locale specific output, and this will change as
	 * you change the locale of the formatter. Call
	 * {@link DateTimeFormatter#withLocale(Locale)} to switch the locale. For
	 * example:
	 * 
	 * <pre>
	 ...
	 * 'L' for long, and 'F' for full. A date or time may be ommitted by
	 * specifying a style character '-'.
	 * <p>
	 * The returned formatter will dynamically adjust to the locale that the
	 * print/parse takes place in. Thus you just call
	 * {@link DateTimeFormatter#withLocale(Locale)} and the
		...
		for (int i = 0; i < length; i++) {
			...
			case 'Y': // year of era (number)
				if (tokenLen == 2) {
					...
					switch (c) {
					case 'x':
						builder.appendTwoDigitWeekyear(new DateTime()
								.getWeekyear() - 30, lenientParse);
						break;
					case 'y':
					case 'Y':
					default:
						builder.appendTwoDigitYear(
								new DateTime().getYear() - 30, lenientParse);
						break;
					}
				} else {
					...
					if (i + 1 < length) {
						indexRef[0]++;
						if (isNumericToken(parseToken(pattern, indexRef))) {
							// If next token is a number, cannot support long
							// years.
							maxDigits = tokenLen;
						}
						indexRef[0]--;
	 ...
	 *            the pattern string
	 * @param indexRef
	 *            a single element array, where the input is the start location
	 *            and the output is the location after parsing the token
	...
	private static DateTimeFormatter createFormatterForStyle(String style) {
		if (style == null || style.length() != 2) {
			throw new IllegalArgumentException("Invalid style specification: "
					+ style);
		}
	 ...
	 * @return the formatter
	 */
	private static DateTimeFormatter createFormatterForStyleIndex(
		...
		synchronized (cStyleCache) {
			...
			if (f == null) {
				...
				StyleFormatter llf = new StyleFormatter(dateStyle, timeStyle,
						type);
	...
	static class StyleFormatter implements DateTimePrinter, DateTimeParser {
		...
		private static final Map<String, DateTimeFormatter> cCache = new ConcurrentHashMap<String, DateTimeFormatter>(); // manual
		// sync
		...
		public int parseInto(DateTimeParserBucket bucket, String text,
				int position) {
		...
		private DateTimeFormatter getFormatter(Locale locale) {
			...
			String key = Integer.toString(iType + (iDateStyle << 4)
					+ (iTimeStyle << 8))
					+ locale.toString();
			DateTimeFormatter f = null;
			// synchronized (cCache) {
		...
		String getPattern(Locale locale) {
			...
			case TIME:
				...
				break;
			case DATETIME:
				f = DateFormat.getDateTimeInstance(iDateStyle, iTimeStyle,
						locale);
				break;
			}
